...
class AuditCSVReaderWriter(private val filePath: String) : IReaderWriter<AuditDto> {
    ...
    override suspend fun overWrite(data: List<AuditDto>): Boolean {
        data.toDataFrame().writeCSV(resolveFile())
        return true
    }

    override suspend fun append(data: List<AuditDto>): Boolean {
        val file = resolveFile()
